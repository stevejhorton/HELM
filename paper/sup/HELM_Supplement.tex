...
\subsection{Derivation of Newton's Constant \texorpdfstring{$G$}{G}}
We derive $G$ from the lattice \emph{without inserting its numerical value}.  Begin with the **only** laboratory inputs: proton mass $m_p$, proton charge radius $r_p$, and speed of light $c$.  From these we obtain the lattice tension
\begin{equation}
\sigma = \frac{\pi}{4}\frac{m_p c^2}{r_p} \quad\text{(QCD scale, 0.89 GeV/fm)}.
\end{equation}
Insert a **static point mass** $M$ at the origin.  The minimal-energy elastic distortion that balances $M$ is the **Keplerian strain field**
\begin{equation}
u(r) = \frac{M c^2}{4\pi\sigma r},
\end{equation}
which satisfies $\nabla^2 u = 0$ for $r>0$ and carries total elastic energy $E = \int (\sigma/2)(\nabla u)^2\,dV = M c^2$.
